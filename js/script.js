...
async function fazerlicaonormal(ra, password, damn) {
    ...
    const pre_getinfo_response = await fetch(`guiguiv4site.vercel.app/getporra?ra=${raEncoded}&password=${passwordEncoded}&porra=${damn}`);
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        let getlessons_response = await fetch(`guiguiv4site.vercel.app/getlesson_normal?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
        if (!getlessons_response.ok) {
            getlessons_response = await fetch(`guiguiv4site.vercel.app/getlesson_normal_2?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
            if (!getlessons_response.ok) {
        ...
        for (const lesson of catapimbas) {
            ...
            try {
                await delay(1000); 
                const dolesson_response = await fetch(`guiguiv4site.vercel.app/dolesso?x_auth_key=${x_auth_key}&room_code=${room_code}&lesson_id=${lesson.id}&porra=${damn}`);
...
async function fazerlicaoatrasada(ra, password, damn) {
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        let getlessons_response = await fetch(`guiguiv4site.vercel.app/getlesson_expired?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
        if (!getlessons_response.ok) {
            getlessons_response = await fetch(`guiguiv4site.vercel.app/getlesson_expired_2?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
            if (!getlessons_response.ok) {
                createAndShowNotification('Erro ao carregar licoes. Verifique sua conexão e tente novamente.');
                return;
            }
